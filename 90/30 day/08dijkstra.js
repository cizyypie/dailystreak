...
function dijkstra(graph, start, end) {
  ...
  while (!pq.isEmpty()) {
    ...
    for (let neighbor in graph[currentKota]) {
      if (visited.has(neighbor)) continue;
      const distance = distances[currentKota] + graph[currentKota][neighbor];

      if (distance < distances[neighbor]) {
        distances[neighbor] = distance;
        previous[neighbor] = currentKota;
